...
test("ceux/celle are properly replaced", () => {
  expect(inclusify("Ceux qui veulent venir peuvent s’inscrire.")).toBe(
    "Celleux qui veulent venir peuvent s’inscrire.",
  );
  expect(inclusify("Celle que j’ai vue était magnifique.")).toBe(
    "Celleux que j’ai vue était magnifique.",
  );
  expect(
    inclusify("Ceux qui ont fini peuvent aider celle qui a besoin d’aide."),
  ).toBe("Celleux qui ont fini peuvent aider celleux qui a besoin d’aide.");
});
